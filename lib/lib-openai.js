...
module.exports.constructOneOffPayload = async function(systemPromptOverride, messageText) {
  ...
  const payload = await this.constructSystemPrompt(systemPromptOverride);
...
module.exports.constructSystemPrompt = async function(systemPromptOverride) {

  const systemPrompt = (systemPromptOverride) ? systemPromptOverride : process.env.OPENAI_PARAM_SYSTEM_PROMPT;
  const payload = [{
    role: 'system',
