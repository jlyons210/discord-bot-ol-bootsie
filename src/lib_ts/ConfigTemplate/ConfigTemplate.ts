  ...
  public validateStartupSettings() {
    ...
    defaults.forEach(setting => {
      ...
      if (userValue) {
        ...
        if (setting.allowedValues) {
          ...
          if (typeof setting.allowedValues !== 'number' && setting.allowedValues.includes(userValue)) {
            ...
            Logger.log(`${setting.name} = ${safeOutput}`, 'info');
          }
          // ...or the user-provided value is not allowed (invalid):
          else {
            Logger.log(`${setting.name}=${userValue} is invalid - allowed value(s): ${setting.allowedValues}`, 'error');
        ...
        // ...or the template allows any value matching its value type (valid):
          ...
          Logger.log(`${setting.name} = ${valueOutput}`, 'info');
        }
        // ...or the user-provided value doesn't match the template's value type (invalid):
        else {
          Logger.log(`${setting.name}=${userValue} is invalid - expected type: ${typeof setting.allowedValues}`, 'error');
          validationFailed = true;
        }
      ...
      // If the user did not provide a value, but the template has a default value (valid/default)
        ...
        Logger.log(`${setting.name} not set - using template default: ${setting.defaultValue}`, 'info');
      ...
      // If the user did not provide a value, but the setting is required (invalid)
        ...
        Logger.log(`${setting.name} not set and is required.`, 'error');
        validationFailed = true;
      }
    });
    ...
    if (validationFailed) {
      Logger.log('Startup settings are not configured correctly. See documentation on GitHub.', 'error');
