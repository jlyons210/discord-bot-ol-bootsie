  ...
  private validateStartupSettings(): void {
    ...
    ConfigTemplate.forEach(setting => {
      ...
      if (userValue !== undefined) {
        if (
          (setting.allowedValues && this.isAllowedValue(userValue, setting.allowedValues))
          || (typeof userValue === typeof setting.allowedValues)
        ) {
          this.settings[setting.name] = userValue;
          process.env[setting.name] = undefined;

          const safeOutput = (setting.secret)
            ? '*'.repeat(10)
            : userValue;

          void this.logger.logInfo(`${setting.name} = ${safeOutput}`);
        }
        // User setting is invalid
        else {
          if (setting.allowedValues && !this.isAllowedValue(userValue, setting.allowedValues)) {
      ...
      // User did not configure an optional setting, use template default
        ...
        this.settings[setting.name] = setting.defaultValue || '';
      }

      // User did not configure a required setting
      else {
        void this.logger.logError(`${setting.name} not set and is required.`);
        validationFailed = true;
      }
    });
